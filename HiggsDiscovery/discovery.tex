\chapter{Higgs Boson Discovery in $ZZ\rightarrow4\ell$}
\label{sec:discovery}
...
\end{center}
\caption[Irreducible $ZZ$ Backgrounds in the $4\ell$ Channel]{Feynman diagrams for the irreducible backgrounds in the $4\ell$ channel. $q\bar{q}\rightarrow 4\ell$ (left) is dominant, while $gg\rightarrow 4\ell$ (right) requires a loop and is subdominant.}
\label{fig:IrreducibleBackFeyn}
\end{figure}

All generated background and signal samples interface with {\tt PYTHIA} to produce hadronization and low-$p_T$ radiation effects. After this stage, every sample was processed through software built on {\tt GEANT4} \cite{Agostinelli:2002hh,GEANT} which simulates the passage of particles through the matter of CMS subdetectors, then reconstructs objects using the same algorithms applied on data, accounting for any pileup effects. For any LO generators, the PDF used for generation is {\tt CTEQ6L} \cite{cteq66}. For NLO generators, {\tt CT10} \cite{Lai:2010vv} is used. 
...
\caption[Dijet Ratio of Gluon Fusion and Weak Vector Boson Fusion as Function of $m_H$]{Dijet ratio of ggF (top row) and VBF (bottom row) as functions of $m_H$. Left to right: $4\mu$, $4e$, $2e2\mu$. There should be very little correlation between the Higgs final state and the dijet ratio, which is exactly what is seen.}
