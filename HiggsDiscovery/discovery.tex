...
All generated background and signal samples interface with {\tt PYTHIA} to produce hadronization and low-$p_T$ radiation effects. After this stage, every sample was processed through software built on {\tt GEANT4} \cite{Agostinelli:2002hh,GEANT} which simulates further decays and geometric acceptances in CMS, then reconstructs objects using the same algorithms applied on data, accounting for any pileup effects. For any LO generators, the PDF used for generation is {\tt CTEQ6L} \cite{cteq66}. For NLO generators, {\tt CT10} \cite{Lai:2010vv} is used. 
